...
## اجرای چند نمونهٔ API

امروز یک کانتینر API با یک پروسهٔ Node اجرا می‌شود. Node تک‌رشته‌ای است، پس یک
هستهٔ CPU سقف کار همزمان است و هر کرش یعنی قطعی کامل تا بالا آمدن دوباره.

**چرا هنوز روشنش نکرده‌ایم:** استقرار روی `docker-compose.hostnet.yml` است —
شبکهٔ میزبان. در این حالت چند کانتینر نمی‌توانند هم‌زمان پورت ۴۰۰۰ را بگیرند،
پس مقیاس افقی یک تغییر توپولوژی است نه یک عدد. با ترافیک فعلی، ریسکش از
فایده‌اش بیشتر بود.

**آنچه از قبل آماده است:**

- شمارندهٔ rate limit روی Redis است، پس سقف‌ها به‌ازای هر پروسه ضرب نمی‌شوند
- صف ایمیل با `BRPOP` کار می‌کند، پس هر job فقط به یک پروسه می‌رسد
- `DB_POOL_MAX` قابل تنظیم است

**وقتی خواستی روشنش کنی:**

1. به هر نمونه `PORT` متفاوت بده (۴۰۰۰، ۴۰۰۱، ۴۰۰۲)
2. در nginx یک `upstream` با همان پورت‌ها بساز و پروکسی را به آن ببر
3. `DB_POOL_MAX` را طوری بگذار که «تعداد نمونه × مقدار آن» زیر `max_connections`
   پستگرس بماند (پیش‌فرض پستگرس ۱۰۰ است)
4. `RETENTION_INTERVAL_HOURS=0` را روی همهٔ نمونه‌ها جز یکی بگذار — کارِ پاک‌سازی
   idempotent است ولی سه بار اجرا شدنش سه برابر بار برای یک بار کار است
5. `pcmax backup` و `pcmax deploy` به نام کانتینر `goh-api` تکیه دارند و باید
   به‌روز شوند
