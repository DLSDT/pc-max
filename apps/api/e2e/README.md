...
بدون اعتبارنامه‌ی ادمین هم اجرا می‌شود؛ تست‌های نشان‌دار با `admin: true`
**رد می‌شوند** (skip) نه شکست. نبودن رمز یک نقص راه‌اندازی است نه خرابی API، و
اگر به‌شکل ۱۷ شکست یکسان گزارش شود تنها شکست واقعی زیرش گم می‌شود.
...
دو نشانه‌ی اختیاری روی هر تست:

| نشانه | یعنی |
|---|---|
| `mode: 'full'` | ردیف می‌سازد یا پاک می‌کند — فقط لوکال |
| `admin: true` | به توکن ادمین نیاز دارد — بدون رمز skip می‌شود |
